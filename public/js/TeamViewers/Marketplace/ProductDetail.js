...
document.addEventListener("DOMContentLoaded", async function () {
    // Obtain UserID & ProductID
    let UserDiv = document.getElementById("UserID");
    UserID = UserDiv.dataset.id;
    let productDiv = document.getElementById("ProductID");
    ProductID = productDiv.dataset.id;

    const CurrentUser = await readDatabase(`SELECT * FROM user WHERE ID = '${UserID}'`);
    Username = CurrentUser[0].Channel;
    Profile = CurrentUser[0].Picture;

    // Generate List Order
    Product_Data = await readDatabase(`SELECT Product, ID AS Penjual, Channel, Title, Deskripsi, Gambar, Tipe, Harga, Review, (SELECT Whatsapp FROM user WHERE ID = Penjual) AS Whatsapps FROM marketplace WHERE Product = ${ProductID}`);
    document.querySelector('.productname').textContent = Product_Data[0].Title;
    document.querySelector('.sellername').textContent = Product_Data[0].Channel;
    document.querySelector('.ratingcount').textContent = Product_Data[0].Review == "" ? "0" : ratings(Product_Data[0].Review);
    document.querySelector('.soldcount').textContent = Product_Data[0].Review == "" ? "0" : Product_Data[0].Review.split('‽').filter(item => item.trim() !== '').length;;
    document.querySelector('.priceamount').textContent = "Rp " + Number(Product_Data[0].Harga).toLocaleString();
    document.querySelector('.descriptiontext').textContent = Product_Data[0].Deskripsi;

    createReview();
    for (let i = 0; i < Product_Data[0].Gambar.split('‽').length; i++) {
        // document.querySelector(`.image-product-1.display-${i}`).src = Product_Data[0].Gambar.split('‽')[i - 1];
        if (Product_Data[0].Gambar.split('‽')[i] != "") {
            $(document).ready(function () {
                $(".div-block-7.list-image").append(`
                    <div class="w-layout-blockcontainer product-image-1 w-container" onclick="setImage('${Product_Data[0].Gambar.split('‽')[i]}')">
                        <img
                            src="${Product_Data[0].Gambar.split('‽')[i]}"
                            loading="lazy"
                            width="60"
                            height="60"
                            sizes="60px"
                            class="image-product-1"
                        />
                    </div>
                `);
            });
    ...
    setImage(Product_Data[0].Gambar.split('‽')[0]);
    createOrder();
    notification();
    ...
    document.getElementById('name-3').addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
          ...
          window.location.href = `/TeamViewers/Marketplace/ShoppingPage/${UserID}`;
        }
    });
});
...
async function buyProduct() {
    await readDatabase(`INSERT INTO transaction VALUES (DEFAULT, 'Process', '${Product_Data[0].Penjual}', '${UserID}', ${Product_Data[0].Product}, '${Product_Data[0].Title}', '${Product_Data[0].Gambar.split('‽')[0]}', ${Amount}, ${Number(Product_Data[0].Harga) * Amount}, CURRENT_TIMESTAMP)`);
    location.reload(true);
    // let payment_link = await createPayment(Product_Data[0].Title, (Number(Product_Data[0].Harga) * Amount).toString());
    // window.open(payment_link, '_blank');
}

function SelectRating(Stars) {
    for (let i = 0; i < 5; i++){
        document.querySelector(`.star${i + 1}`).src = i < Stars ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png";
    }
    StarSelected = Stars;
}

async function FinishOrder() {
    let kalimat_review = document.querySelector('.textarea-new.w-input.reviewersnya').value;
    await readDatabase(`UPDATE marketplace SET Review = '${Product_Data[0].Review}${UserID}§${Username}§${Profile}§${StarSelected}§${kalimat_review}‽' WHERE Product = ${ProductID}`);
    await readDatabase(`UPDATE transaction SET Status = 'Finished' WHERE ID = ${TargetOrder}`);
    var element = document.querySelector(".popup");
    if (element) {
        element.parentNode.removeChild(element);
        element.parentNode.removeChild(document.querySelector(`.div-block-12.transaction${TargetOrder}`));
        createReview();
    }
}

async function createOrder() {
    Order_Data = await readDatabase(`SELECT ID, Status, ProductID AS Barang, (SELECT Title FROM marketplace WHERE Product = Barang) AS Nama, Amount, Price FROM transaction WHERE ProductID = ${ProductID} && Buyer = '${UserID}' && Status = 'Process'`);
    for (let i = 0; i < Order_Data.length; i++) {
        $(document).ready(function () {
            $(".area.3").append(`
                <div class="div-block-12 transaction${Order_Data[i].ID}">
                    <div class="form-block-3-new w-form">
                        <form
                            ...
                            data-wf-element-id="ba3dac0d-0e56-1901-d19d-32f35d2add62"
                        >
                            <textarea
                                id="CatatamPenjual-2"
                                ...
                                class="textarea-new w-input"
                            >${Order_Data[i].Amount} x ${Order_Data[i].Nama}</textarea>
                        </form>
                    ...
                    <div class="form-block-3-new-2 w-form">
                        <form
                            ...
                            method="get"
                            data-wf-page-id="666423c230a62eac90cd44a4"
                            data-wf-element-id="1375288c-970e-82a5-741e-2f119cc7c8da"
                        >
                            <textarea
                                id="CatatamPenjual-2"
                                name="CatatamPenjual-2"
                                ...
                                data-name="Catatam Penjual 2"
                                placeholder="Catatan Penjual"
                                class="textarea-new-3 w-input"
                            >${"Rp " + Number(Order_Data[i].Price).toLocaleString()}</textarea>
                        </form>
                        <div class="w-form-done">
                            <div>
                                Thank you! Your submission has been received!
                    ...
                    <div class="cancel-done">
                        <a href="#" class="button-2-copy w-button">Cancel</a
                        ><a href="#" class="button-3a w-button" onclick="CompleteOrder('${Order_Data[i].ID}')">Done</a>
                    </div>
                </div>
            `);
        });
...
async function createReview() {
    var reviews = Product_Data[0].Review.split('‽');
    var Comments = reviews.filter(function (review) {
        return review.trim() !== "";
    });

    $('.w-layout-vflex.reviewarea').empty();
    if (Product_Data[0].Review == "") { return; }
    for (let i = 0; i < Comments.length; i++) {
        $(document).ready(function () {
            $(".w-layout-vflex.reviewarea").append(`
                <div class="w-layout-blockcontainer reviewbox w-container">
                    <div class="w-layout-hflex flex-block-24">
                        <div class="w-layout-hflex flex-block-26">
                            <img
                                src="${Comments[i].split('§')[2]}"
                                loading="lazy"
                                width="250"
                                alt=""
                                class="image-10"
                            ...
                            >
                                <h1 class="heading-10">${Comments[i].split('§')[1]}</h1>
                                <div class="w-layout-hflex">
                                    <img
                                        src="${Number(Comments[i].split('§')[3]) >= 1 ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png"}"
                                        loading="lazy"
                                        width="70"
                                        ...
                                        class="image-7"
                                    /><img
                                        src="${Number(Comments[i].split('§')[3]) >= 2 ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png"}"
                                        loading="lazy"
                                        width="70"
                                        sizes="(max-width: 767px) 18px, (max-width: 991px) 2vw, 18px"
                                        alt=""
                                        class="image-7"
                                    /><img
                                        src="${Number(Comments[i].split('§')[3]) >= 3 ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png"}"
                                        loading="lazy"
                                        width="70"
                                        sizes="(max-width: 767px) 18px, (max-width: 991px) 2vw, 18px"
                                        alt=""
                                        class="image-7"
                                    /><img
                                        src="${Number(Comments[i].split('§')[3]) >= 4 ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png"}"
                                        loading="lazy"
                                        width="70"
                                        sizes="(max-width: 767px) 18px, (max-width: 991px) 2vw, 18px"
                                        alt=""
                                        class="image-7"
                                    /><img
                                        src="${Number(Comments[i].split('§')[3]) >= 5 ? "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6601a1754007075bd039b498_star.png" : "https://cdn.prod.website-files.com/65fc592e25023f3e5548c715/6665465ef8f6a2dd61cbb2df_star%20(1).png"}"
                                        loading="lazy"
                                        width="70"
                                        sizes="(max-width: 767px) 18px, (max-width: 991px) 2vw, 18px"
                                        alt=""
                        ...
                        <p class="paragraph-6">${Comments[i].split('§')[4]}</p>
                    </div>
                </div>
            `);
        });
...
function chat_seller() {
    window.open(`https://wa.me/${Product_Data[0].Whatsapps}`, "_blank");
}

function ModifyAmount(Condition) {
    if (Condition == "+") {
        Amount++;
    } else {
        if (Amount > 0) {
    ...
    document.querySelector('.text-block-5.amountbuy').textContent = Amount.toString();
    document.querySelector('.totalharga1.nilai').textContent = `Rp ${(Number(Product_Data[0].Harga) * Amount).toLocaleString()}`;
}

function changeImage(Turn) {
    if (Turn == "Left") {
        if (CurrentImage == 0) {
            CurrentImage = Product_Data[0].Gambar.split('‽').length - 1;
        } else {
            CurrentImage--;
        }
    } else {
        if (CurrentImage == Product_Data[0].Gambar.split('‽').length - 1) {
            CurrentImage = 0;
        } else {
            CurrentImage++;
        }
    }
    setImage(Product_Data[0].Gambar.split('‽')[CurrentImage]);
}

function setImage(Image) {
    document.querySelector(`.w-layout-blockcontainer.imagereview.w-container`).style.backgroundImage = `url(${Image})`;
}
