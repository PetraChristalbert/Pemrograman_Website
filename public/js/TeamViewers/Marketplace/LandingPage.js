...
async function checkUser() {
    Channel = document.querySelector('.text-field-3.w-input').value;
    ID = document.querySelector('.text-field-4.w-input').value;
    if (ID.length >=4) {
        const User_Data = await readDatabase(`SELECT * FROM user WHERE Channel = '${Channel}' && ID LIKE '%${ID}%'`);
        if (User_Data[0].Channel == Channel) {
            ID = User_Data[0].ID;
...
function openMarketplace() {
    window.location.href = `/TeamViewers/Marketplace/ShoppingPage/${ID}`;
}

function openCentre() {
    window.location.href = `/TeamViewers/Marketplace/SellerPage/${ID}`;
}
