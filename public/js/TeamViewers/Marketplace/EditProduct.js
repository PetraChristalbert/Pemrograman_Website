...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    user_credential = await readDatabase(`SELECT * FROM user WHERE ID = '${UserID}'`);

    Product_Data = await readDatabase(`SELECT Product, ID, Title, Deskripsi, Gambar, Tipe, Harga FROM marketplace WHERE Product = ${ProductID}`);
    if (Product_Data.length == 0) {
        const New_Product = await readDatabase(`SELECT COUNT(*) AS NewItem FROM marketplace`);
        ProductID = `${Number(New_Product[0].NewItem) + 1}`;
        buat_baru = true;
    }
    if (ProductID != "0") {
        document.querySelector('.textarea-new-3.w-input.Title').value = Product_Data[0].Title;
        gantiTipe(Product_Data[0].Tipe);
        document.querySelector('.textarea-new-4.w-input.Harga').value = Product_Data[0].Harga;
        document.querySelector('.textarea-new.w-input.deksripsi-product').value = Product_Data[0].Deskripsi;
        let pre_gambar = Product_Data[0].Gambar.split('‽');
        for (let i = 0; i < pre_gambar.length; i++) {
            Images[i] = pre_gambar[i];
            document.querySelector(`.picture1-${i + 1}`).src = pre_gambar[i];
        }
        buat_baru = false;
    }
});

function currentImageSelected(Number) {
    ImageSelect = Number;
}

async function confirmProduct() {
    let Title_Produk = document.querySelector('.textarea-new-3.w-input.Title').value;
    let Tipe_Produk = document.querySelector('.tipe-terpilih').textContent;
    let Harga_Produk = document.querySelector('.textarea-new-4.w-input.Harga').value;
    let Deskripsi_Produk = document.querySelector('.textarea-new.w-input.deksripsi-product').value;
    let Gambar_Produk = Images.filter(image => image !== "").join('‽');
    // document.querySelector('.textarea-new-3.w-input.Title').value = `INSERT INTO marketplace VALUES (${ProductID}, '${UserID}', '${user_credential[0].Channel}', '${Title_Produk}', '${Deskripsi_Produk}', '${Gambar_Produk}', '${Tipe_Produk}', ${Harga_Produk}, '', CURRENT_TIMESTAMP)`;
    // document.querySelector('.tipe-terpilih').textContent = buat_baru.toString();
    // return;
    if (buat_baru) {
        await readDatabase(`INSERT INTO marketplace VALUES (${ProductID}, '${UserID}', '${user_credential[0].Channel}', '${Title_Produk}', '${Deskripsi_Produk}', '${Gambar_Produk}', '${Tipe_Produk}', ${Harga_Produk}, '', CURRENT_TIMESTAMP)`);
    } else {
        await readDatabase(`UPDATE marketplace SET Title = '${Title_Produk}', Deskripsi = '${Deskripsi_Produk}', Gambar = '${Gambar_Produk}', Tipe = '${Tipe_Produk}', Harga = ${Harga_Produk} WHERE Product = ${ProductID}`);
    }
    window.location.href = `/TeamViewers/Marketplace/SellerPage/${UserID}`;
}
...
function gantiTipe(Tipes) {
    document.querySelector('.tipe-terpilih').textContent = Tipes;
    Product_Tipes = Tipes;
}
...
function handleFileSelection(event) {
    ...
    if (file) {
        ...
        document.querySelector(`.picture1-${ImageSelect}`).src = url;
        Images[ImageSelect] = `https://ums.blue/File_Hosting/Product.${UserID}.${ProductID}.${ImageSelect}.png`;
        let formData = new FormData();
        formData.append('file', file);
        formData.append('customFileName', `Product.${UserID}.${ProductID}.${ImageSelect}.png`);

        fetch('https://ums.blue/php/upload.php', {
            method: 'POST',
            body: formData
        })
      ...
      document.querySelector('.textarea-new-3.w-input.Title').textContent = "Not Found";
    }
  }
...
Webflow.require("ix2").init({
    events: {
        e: {
            ...
            target: {
                id: "6640cbbbaa746925b0543431|787dfa4e-2d85-9dc1-04cd-2972e1b58717",
                appliesTo: "ELEMENT",
                styleBlockIds: [],
            },
            targets: [
                    ...
                    actionItems: [
                        {
                            ...
                            config: {
                                ...
                                target: {
                                    ...
                                    id: "6640cbbbaa746925b0543431|787dfa4e-2d85-9dc1-04cd-2972e1b58718",
                                },
                                value: "flex",
                            },
                        },
